...
export const countTokens = (text: string, model: string): number => {
  let avgCharsPerToken: number = REVIEW_CONSTANTS.DEFAULT_CHARS_PER_TOKEN;

  // Adjust based on model type (rough estimates)
  if (model.includes("claude")) {
    avgCharsPerToken = REVIEW_CONSTANTS.CLAUDE_CHARS_PER_TOKEN;
  } else if (model.includes("gpt-4")) {
    avgCharsPerToken = REVIEW_CONSTANTS.GPT4_CHARS_PER_TOKEN;
  }
...
const expandToFunctionBoundaries = (
  ...
  const functionPatterns =
    patterns[ext as keyof typeof patterns] || patterns.default;
  ...
  for (
    let i = minLine - 1;
    i >= Math.max(0, minLine - REVIEW_CONSTANTS.FUNCTION_SEARCH_RANGE);
    i--
  ) {
    const line = lines[i];
  ...
  for (
    let i = expandedStart - 1;
    i <
    Math.min(lines.length, maxLine + REVIEW_CONSTANTS.FUNCTION_SEARCH_RANGE);
    i++
  ) {
    const line = lines[i];
...
const getContextualContent = async (
  ...
  try {
    ...
    const minLine = Math.max(
      1,
      Math.min(...ranges.map((r) => r.start)) - REVIEW_CONSTANTS.CONTEXT_LINES
    );
    const maxLine = Math.min(
      lines.length,
      Math.max(...ranges.map((r) => r.end)) + REVIEW_CONSTANTS.CONTEXT_LINES
    );

    // For small files (<300 lines), include the entire file
    if (lines.length <= REVIEW_CONSTANTS.SMALL_FILE_THRESHOLD) {
      return fullContent;
    }

    // Try to include complete function/class boundaries for better context
    const adjustedRange = expandToFunctionBoundaries(
      lines,
      minLine,
      maxLine,
      file.filename
    ...
    const contextualLines = lines.slice(
      adjustedRange.start - 1,
      adjustedRange.end
    );
    const numberedLines = contextualLines.map(
      (line, index) => `${adjustedRange.start + index}: ${line}`
    );

    return numberedLines.join("\n");
  } catch (error) {
    console.warn(
      `Could not get contextual content for ${file.filename}: ${error}`
    );
    return undefined;
  }
};
...
export const chunkDiff = async (
  ...
  const maxChunkSize = REVIEW_CONSTANTS.MAX_CHUNK_SIZE;

  // Filter out ignored files first
  const validFiles = diff.filter(
  ...
  for (const file of validFiles) {
    ...
    if (octokit && context && sha) {
      const contextualContent = await getContextualContent(
        file,
        octokit,
        context,
        sha
  ...
  for (const { file, content, size } of fileData) {
    ...
    if (
      currentChunk.fileChanges.length === 0 ||
...
export const processIncrementalDiff = (
  ...
  if (incrementalDiff.newCommits.length === 0) {
    return {
      shouldReview: false,
      message:
        "🔄 **Incremental Review**: No new commits to review since last review.",
    };
  }
